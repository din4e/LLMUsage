...
在仪表盘增加独立的「最近变化」面板。用户先单独选择一个 provider 实例，再选择一个指标，查看该实例最近两条可比历史采样之间的变化量及最近一段采样曲线，不把多个 provider 或多个指标混在一起。
...
- 同一选择下显示最近最多 12 个有效采样的紧凑曲线；请求、Token、成本只绘制最新自然日，余额允许跨日。
- 曲线至少需要两个点，使用真实采样值和时间顺序，不补零、不插值；每个点提供可聚焦的时间和值说明。
...
- `src/domain.ts`：选择最近两条可比采样、最近曲线点并计算变化量的纯函数与类型。
- `src/domain.test.ts`：跨日、缺失指标、余额正负变化、多实例隔离和曲线截断测试。
...
const points = selectProviderChangeSeries(records, instanceId, metric, 12);
if (!change || points.length < 2) renderEmptyChange("至少需要两条可比采样");
else renderChange(change, points);
```
...
4. 不支持或样本不足时显示空状态。
5. 重启后仍从已有历史恢复最近变化。
6. 曲线准确展示最近最多 12 个有效采样，累计指标不跨日、余额可跨日，每个点可由键盘聚焦。
7. 全部测试和生产构建通过，版本保持 v0.1.5。
