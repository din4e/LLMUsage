...
- 批量导入/导出（供应商页顶栏）：「完整备份」导出版本化 JSON，含明文凭据串、实例备注与最近剩余状态（余额、额度窗口、冷却、最后同步时间），导出前弹明文警告；「状态报告」导出同构文件但绝不含凭据键，可安全分享。导入不做联网验证，仅离线校验凭据格式；实例 id 冲突自动加 `_N` 后缀，永不覆盖本机已有实例；备注跟随新 id 写入本地设置。文件经由系统保存/打开对话框选择，路径之外不做任何隐式读写；导入不写快照缓存、日汇总或日志，返回结果不含凭据字节。
...
- 「今日消耗」优先使用官方成本接口；仅有人民币余额时按余额差分估算并标注「估算」，充值与赠款不计入（截断为非负），基线取前一日最晚余额采样。
...
6. 官方或本地用量存在时正确显示当日统计；缓存跨重启保留最近一次同步结果与按供应商划分的每日汇总，并支持最近 7 日、最近 30 日和全部趋势查询。某实例同步失败时该行立即标记失败态（数值变灰、提示变红并悬停展示完整错误），明示当前展示的是缓存数据，恢复成功后自动解除。
