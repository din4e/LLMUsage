...
impl CommandError {
    ...
    fn history_write_failed() -> Self {
        Self {
            code: "HISTORY_WRITE_FAILED",
            message: "本地用量历史写入失败，历史趋势可能已重置；如持续出现请检查磁盘空间与目录权限",
        }
    }
...
/// store: a failing local history write must not masquerade as a keychain
/// problem (the network sync itself already succeeded at this point).
fn record_daily_usage(
    ...
    daily_usage_history(app)?
        .upsert(record)
        .map_err(|_| CommandError::history_write_failed())
}
...
fn cache_snapshot<T: Serialize>(
    ...
    let value = serde_json::to_value(snapshot).map_err(|_| CommandError::cache_write_failed())?;
    snapshot_cache(app)?
        .save(provider_id, kind, value)
        .map_err(|_| CommandError::cache_write_failed())
}
...
#[tauri::command(rename_all = "camelCase")]
pub fn load_daily_usage(app: tauri::AppHandle) -> Result<Vec<DailyUsageRecord>, CommandError> {
    daily_usage_history(&app)?
        .load()
        .map_err(|_| CommandError::history_unavailable())
}
...
pub async fn configure_online_provider(
    ...
    // Range values are generated by the frontend; a bad one is a caller bug,
    // not a credential problem, and gets its own error instead of the
    // provider-specific key-check guidance.
...
/// leave them group/world-readable, so tighten to owner-only; Windows relies
/// on the user-profile directory ACL. Best effort — the file is already
/// written by the time this runs.
#[cfg(unix)]
fn restrict_backup_permissions(path: &str) {
