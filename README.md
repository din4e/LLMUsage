...
v0.1.5 新增「今日资金消耗」：每个账号行内展示今日消耗金额，仪表盘顶部新增汇总磁贴。数据采用混合口径——OpenAI 与 Claude Code 使用官方成本接口的真实数据；其余人民币余额账号（DeepSeek、硅基流动、PPIO、Kimi 等）按余额变化估算，充值与赠款不计入、不会出现负数，估算值单独标注口径。删除账号后立即退出汇总，但历史趋势保持完整。
...
| v0.1.5 | 今日资金消耗：按账号展示官方成本或余额差分估算（充值不计入），顶部磁贴汇总 |
