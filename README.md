...
![版本](https://img.shields.io/badge/version-v0.1.7-087b5d)
...
v0.1.7 是一次全面的稳健性版本（依据 [2026-09-24 代码审计](docs/audit/2026-09-24-code-audit.md)修复 22 项发现）：本地用量历史与快照缓存损坏后自动隔离并自愈，不再导致同步永久失败或误报凭据管理器错误；本地文件写入改为 fsync + 换名的原子路径并支持崩溃恢复。OpenAI / Claude Code / Anthropic 用量接口跟随分页翻页（上限 10 页，截断时标注「数据不完整」），大组织数据不再静默低估；美元汇率换算结果缓存 12 小时，不再每次同步访问 frankfurter.app；Kimi Global 改按美元展示且不计入人民币合计。GLM「今日」窗口改按北京时间构造，与 GLM 计费日对齐；GLM / Kimi 汇总额度改取最紧张窗口，GLM 额度窗口缺失重置时间不再中断同步。界面修正：在线与 GLM 行的冷却倒计时统一每 30 秒刷新，明细卡「重置星期」取重置时刻而非当前时间，合计余额趋势与「今日消耗」一致地剔除已删除实例，主题选择重启保持，拖拽句柄异常释放后整行不再保持可拖，同步失败行明确标记并显示缓存数据。跨平台：Linux 构建修复（keyring 依赖改为非 Windows target），macOS 凭据新增实例注册表使列举/导出/导入判重不再只认 DPAPI 文件，完整备份在 Unix 下收紧为 0600 权限。

> v0.1.5 新增「今日资金消耗」：每个账号行内展示今日消耗金额，仪表盘顶部新增汇总磁贴。数据采用混合口径——OpenAI 与 Claude Code 使用官方成本接口的真实数据；其余人民币余额账号（DeepSeek、硅基流动、PPIO、Kimi 等）按余额变化估算，充值与赠款不计入、不会出现负数，估算值单独标注口径。新增独立的「最近变化」面板，可按 provider 实例单独选择请求、Token、余额或成本，比较最近两条可比采样并绘制最近 12 个有效采样的曲线；累计指标不会跨日误算，余额可跨日显示增减。本版本同时兼容 GLM 新版 `CREDIT_LIMIT` 与旧版 `TOKENS_LIMIT` 额度响应，并展示 5 小时、每周等全部可识别窗口。
...
| v0.1.5 | 今日资金消耗；按 provider 实例查看最近请求/Token/余额/成本变化；兼容 GLM 新旧额度类型与多窗口响应 |
| v0.1.7 | 稳健性版本：本地历史/缓存损坏自愈、用量接口分页跟随、汇率缓存、Kimi Global 美元口径、GLM 北京时间窗口、倒计时与主题等界面修正、Linux/macOS 构建与凭据枚举补齐 |
...
- Provider 选择器会按当前指标过滤；余额型供应商不会出现在 Token 选择中，也不会被误报为零消耗或显示通用的“尚未同步”提示。
...
- **Provider 最近变化**：按账号实例单独选择请求、Token、余额或成本，展示当前值、上次值、带符号变化量、采样区间和最近 12 个有效采样的曲线。
...
- Windows 是当前首要发布平台；macOS/Linux 的系统凭据实现已补齐（keyring 存储 + 实例注册表枚举），但打包与真机验证仍待进行。
