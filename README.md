...
![版本](https://img.shields.io/badge/version-v0.1.3-087b5d)
...
v0.1.3 将用量历史采样细化到 15 分钟粒度：开启 15 分钟自动同步后，同一天内的多次采样不再互相覆盖，新增「今日」趋势视图展示当天 96 个 15 分钟槽位的 Token 累计曲线；超过 30 天的明细自动合并为每日一条以保留长期历史。供应商同步改为并行执行，单个供应商失败不再拖累其他供应商。
